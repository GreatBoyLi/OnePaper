...
# PRETRAINED_MODEL_PATH = "../checkpoints/Epoch:23-RMSE:0.0539-MAE:0.0197-MAPE:14.29%-R:97.94%.pth"
...
def main():
    ...
    if rank == 0:
        ...
        with open(csv_path, mode='w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            # 写入表头：包含所有指标
            writer.writerow(['Epoch', 'Train_Loss', 'Val_Loss', 'RMSE', 'MAE', 'MAPE(%)', 'R(%)'])

            # 写入数据：使用 zip 同时遍历所有列表
            # enumerate 用于获取 epoch 序号 (从 1 开始)
            for i, (t_loss, v_loss, rmse, mae, mape, r) in enumerate(
                    zip(train_loss_history, val_loss_history, rmse_hist, mae_hist, mape_hist, r_hist)
            ):
                writer.writerow([i + 1, t_loss, v_loss, rmse, mae, mape, r])

        logger.info(f"📄 完整训练历史已保存至: {csv_path}")
