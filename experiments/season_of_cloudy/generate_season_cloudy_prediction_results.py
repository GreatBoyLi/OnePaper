...
SAVE_DIR = "./season_cloudy_prediction_results"
...
# 5. 自动筛选 Cloudy 代表日
# =========================
def select_cloudy_day(result_csv_path, csi_low=0.4, csi_high=0.8, zenith_threshold=85):
    df = pd.read_csv(result_csv_path, parse_dates=["Time"])

    if "CSI" not in df.columns:
        print(f"跳过 Cloudy 日筛选：{result_csv_path} 中没有 CSI 列")
        return None

    if "Solar_Zenith_Raw" not in df.columns:
        print(f"跳过 Cloudy 日筛选：{result_csv_path} 中没有 Solar_Zenith_Raw 列")
        return None
    ...
    daily_stats = (
        ...
        .agg(
            ...
            Cloudy_Ratio=("CSI", lambda x: np.mean((x >= csi_low) & (x < csi_high)))
        )
    )
    ...
    # 优先选择 Cloudy 样本占比最高的日期；
    # 如果占比一样，则选择平均 CSI 更接近 0.6 的日期。
    daily_stats["CSI_Distance_To_Cloudy_Center"] = np.abs(daily_stats["Mean_CSI"] - 0.6)

    daily_stats = daily_stats.sort_values(
        by=["Cloudy_Ratio", "CSI_Distance_To_Cloudy_Center", "Sample_Number"],
    ...
    print(
        f"Cloudy 代表日：{selected_date}, "
        f"Mean CSI = {selected_row['Mean_CSI']:.4f}, "
        f"Cloudy Ratio = {selected_row['Cloudy_Ratio']:.2%}, "
...
def main():
    ...
    for item in SEASON_FILES:
        ...
        selected_date = select_cloudy_day(save_path)
        if selected_date is not None:
            selected_cloudy_days[season_name] = selected_date
    ...
    print("\n各季节自动筛选的 Cloudy 代表日：")
    for season, date_str in selected_cloudy_days.items():
        print(f"{season}: {date_str}")

    if len(selected_cloudy_days) > 0:
        cloudy_day_path = os.path.join(SAVE_DIR, "selected_cloudy_days.csv")
        pd.DataFrame(
            [
                {"Season": season, "Date": date_str}
                for season, date_str in selected_cloudy_days.items()
            ]
        ).to_csv(cloudy_day_path, index=False, encoding="utf-8-sig")

        print(f"\nCloudy 代表日已保存：{cloudy_day_path}")
