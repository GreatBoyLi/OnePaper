...
# 0. 路径配置
# =========================
RESULT_DIR = "./season_rainy_prediction_results"
SAVE_DIR = "./season_rainy_day_figures"
...
RAINY_DAY_FILE = os.path.join(RESULT_DIR, "selected_rainy_days.csv")
...
# 2. 读取自动筛选的 Rainy 代表日
# =========================
def load_selected_rainy_days(rainy_day_file):
    if not os.path.exists(rainy_day_file):
        raise FileNotFoundError(f"找不到 Rainy 代表日文件: {rainy_day_file}")

    df = pd.read_csv(rainy_day_file)
...
# 5. 绘制春夏秋冬 2×2 图
# =========================
def plot_season_rainy_days(season_dates):
    ...
    png_path = os.path.join(SAVE_DIR, "season_rainy_day_power_prediction.png")
    pdf_path = os.path.join(SAVE_DIR, "season_rainy_day_power_prediction.pdf")
    svg_path = os.path.join(SAVE_DIR, "season_rainy_day_power_prediction.svg")
...
# =========================
def main():
    season_dates = load_selected_rainy_days(RAINY_DAY_FILE)

    print("Selected Rainy dates:")
    ...
    for season in required_seasons:
        if season not in season_dates:
            raise ValueError(f"selected_rainy_days.csv 中缺少 {season} 的代表日。")

    plot_season_rainy_days(season_dates)
